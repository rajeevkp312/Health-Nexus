...
export function DoctorConfirmedAppointments() {
  ...
  const fetchConfirmedAppointments = async () => {
    try {
      ...
      if (response.data.msg === "Success") {
        ...
        const confirmedAppointments = allAppointments.filter(app => normalizeStatus(app.status) === "confirmed");
        const enriched = await enrichAppointmentsWithPatient(confirmedAppointments);
        ...
        <div className="flex-1 px-2 py-1 sm:p-4 lg:p-6 pt-20 lg:pt-6 pb-16 lg:pb-6 space-y-2 sm:space-y-4 overflow-x-hidden max-w-full">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-2 sm:p-4 mx-1 sm:mx-0">
            <div className="flex items-center space-x-2 sm:space-x-4">
              <div className="bg-green-600 rounded-full p-1.5 sm:p-3 flex-shrink-0">
                <CheckCircle className="h-4 w-4 sm:h-8 sm:w-8 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h1 className="text-base sm:text-2xl font-bold text-gray-900 truncate">Confirmed Appointments</h1>
                <p className="text-xs sm:text-base text-gray-600 truncate">Total: {appointments.length} appointments confirmed</p>
              </div>
            </div>
          </div>
          {appointments.length > 0 ? (
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-600 text-white border border-green-700">
                          <CheckCircle className="h-4 w-4 mr-1 text-white" />
                          Confirmed
                        </span>
                        <div className="flex items-center space-x-2 text-sm text-gray-500">
                          <Clock className="h-4 w-4" />
                          <span>Scheduled for {appointment.date}</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-blue-50 rounded-lg p-4 mb-6 border border-blue-200">
                        <h4 className="font-medium text-blue-900 mb-2">Appointment Purpose</h4>
                        <p className="text-blue-800">{appointment.reason || appointment.description || 'General Consultation'}</p>
                      </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-4">
                        ...
                        <ul className="text-sm text-gray-600 space-y-1">
                          ...
                          <li>• Prepare necessary examination tools</li>
                          <li>• Allocate appropriate time for consultation</li>
                        </ul>
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
              ...
              <p className="text-gray-500">You don't have any confirmed appointments at the moment.</p>
            </div>
          )}
          </div>
        </div>
